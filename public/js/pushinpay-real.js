// SyncPay Integration
// Versão: 2.0.0 - SyncPay Integration (migrado de PushinPay)
console.log('%c🚀 SyncPay Integration v2.0.0', 'color: #ff6b35; font-size: 16px; font-weight: bold;');
console.log('%c✅ Sistema SyncPay carregado!', 'color: #4ade80; font-size: 12px;');
console.log('🔗 API: https://api.syncpayments.com.br');
...
const PushinPayReal = {
  ...
  async criarPix(client = null, currency = 'BRL') {
    try {
      ...
      console.log('✅ Transação criada com sucesso via SyncPay:', data);
      ...
      if (paymentStatus === 'refused' || paymentStatus === 'failed') {
        console.error('❌ ATENÇÃO: Transação foi RECUSADA pelo SyncPay!');
        const errorMessage = data.message || data.error || 'Transação recusada pelo SyncPay.';
      ...
      if (isBase64Image) {
        ...
        console.log('✅ QR Code exibido (base64 do SyncPay)');
      } else if (pixCode) {
      ...
      if (identifier) {
        ...
        console.warn('⚠️ Transaction Hash não encontrado na resposta da API SyncPay:', data);
      }
